...
export function makeTreeView(data: INode[]) {
  ...
  keys.forEach(child => {
    ...
    if (parentId) {
      ...
      // @ts-ignore
      newData = newData.map((d: INode) =>
        d && d.children && d.id == parentId
          ? { ...d, children: [...d.children, childData] }
          : d.id == child
          ? `false`
          : d,
      );
  ...
  const filterdData = newData.filter(item => typeof item === 'object');
...
export const getFormFields = (formType: string) => {
  if (childFormFields[formType]) {
    // @ts-ignore
    const fieldsWithValues = Object.values(childFormFields[formType]).reduce(
      (acc: any, field: ITaskForm) => {
        acc[`${field.title}`] = '';
        return acc;
      },
      {},
...
function generateHourlyRanges(dateString) {
  ...
  // Full-day range
  hourlyRanges.push({
    id: `${hourlyRanges[0].id.split('_')[0]}_${
      hourlyRanges[hourlyRanges.length - 1].id.split('_')[1]
    }`,
    name: '7 AM to 11 PM',
  });
      ...
      result = result.map((t, i) => {
        if (t.name == item.name) {
          let modifiedTask = {
            ...
            mergeId:
              (t.id ? t.id : 'null') + '|---|' + (item.id ? item.id : 'null'),
            assignedTime: `${existTask.assignedTime} |---| ${getTime}`,
            time_spent:
              (t.time_spent ? t.time_spent : 0) +
              '|---|' +
              (item.time_spent ? item.time_spent : 0),
            estimated_time:
              (t.estimated_time ? t.estimated_time : 0) +
              '|---|' +
              (item.estimated_time ? item.estimated_time : 0),
            wasted_time:
              (t.wasted_time ? t.wasted_time : 0) +
              '|---|' +
              (item.wasted_time ? item.wasted_time : 0),
            reason_for_satisfaction:
              (t.reason_for_satisfaction ? t.reason_for_satisfaction : 'null') +
              '|---|' +
              (item.reason_for_satisfaction
                ? item.reason_for_satisfaction
                : 'null'),
            description:
            ...
            focus_rate:
              `${t.focus_rate ? t.focus_rate : 0}` +
              '|---|' +
              `${item.focus_rate ? item.focus_rate : 0}`,
            satisfaction_rate:
              `${t.satisfaction_rate ? t.satisfaction_rate : 0}` +
              '|---|' +
              `${item.satisfaction_rate ? item.satisfaction_rate : 0}`,
          };
          return modifiedTask;
        } else {
          return t;
...
export const getNumbers = (
  ...
  const utilizationPerformance = parseFloat(
    `${(
      parseFloat(`${getSum(`${time_spent}`) * 100}`) /
      (getSum(`${time_spent}`) + getSum(`${wasted_time}`))
    ).toFixed(2)}`,
  );
  const estimationPerformance = parseFloat(
    `${(
      parseFloat(`${getSum(`${estimated_time ?? time_spent}`) * 100}`) /
      (getSum(`${time_spent}`) + getSum(`${wasted_time}`))
    ).toFixed(2)}`,
  );
  const properWorkPerformance =
    100 -
    parseFloat(
      `${(
        parseFloat(`${getSum(`${taken_extra_time_to_finish ?? 0}`) * 100}`) /
        (getSum(`${time_spent}`) + getSum(`${wasted_time}`))
      ).toFixed(2)}`,
    );
  ...
  const priorityRate =
    ...
    parseFloat(
      (
        (calculateShortFallTime(`${task_name}`, `${time_spent}`) * 100) /
        dedicatedTime[`${task_name}`]
      ).toFixed(2),
    );

  let total_performance = 0;
  if (dedicatedTime[`${task_name}`]) {
    total_performance = parseFloat(
      parseFloat(
        `${
          priorityRate * 0.05 +
          satisfactionRate * 0.1 +
          estimationPerformance * 0.8 +
          utilizationPerformance * 0.05
        }`,
      ).toFixed(2),
    );
  } else {
...
export function calculateTimeDistribution(tasks: INode[]): TimeDistribution[] {
  try {
    ...
    tasks.forEach(task => {
      if (
        ['', 'null', 'undefined', undefined].includes(`${task.category}`) ||
        !task.time_spent
      )
        return;

      const timeSpent = parseTimeSpent(
        `${task.time_spent}|---|${task.wasted_time}`,
      );
      if (timeSpent === 0) return;

      if (!categoryMap.has(parseInt(task.category))) {
        categoryMap.set(parseInt(task.category), {
...
export function formatDateFromTimestamp(timestamp: string): string {
  try {
    ...
    console.log('Error in formatDateFromTimestamp: ', error);
    return '';
  }
}
...
export function mergeObjects(data:any) {
  ...
  data.forEach((item:any) => {
      for (const key in item) {
          if (!mergedResult[key]) {
            if (key == 'description') {
              mergedResult[key] = `${item['name']}(${getSum(`${item['time_spent']}|---|${item['wasted_time']}`)}m) :- ${item[key]}`;
            }else{
  ...
  return mergedResult;
    ...
    console.log("Error in mergeObjects:",error);
    return {}
  }
